...
const seedDemoData = async () => {
  ...
  for (const od of orderDefinitions) {
    ...
    const order = new Order({
      ...
      courierName: od.orderType === 'delivery' && od.status === 'delivered' ? 'David Martinez' : null,
      courierPhone: od.orderType === 'delivery' && od.status === 'delivered' ? '+14155550301' : null,
  ...
  const notifications = [
    { userId: customers[0]._id, title: 'Order Delivered! 🎉', body: 'Your order from Burger Palace has been delivered. Enjoy your meal!', channel: 'push', type: 'order_update', actionUrl: `/orders/${orders[0]._id}`, isRead: true, readAt: new Date(now - 24 * 24 * 60 * 60 * 1000), sentAt: new Date(now - 25 * 24 * 60 * 60 * 1000), metadata: { orderId: orders[0]._id.toString() } },
    { userId: customers[0]._id, title: '20% off your next order!', body: 'Use code WELCOME20 for 20% off. Limited time offer!', channel: 'in_app', type: 'promotion', actionUrl: '/', isRead: false, sentAt: new Date(now - 3 * 24 * 60 * 60 * 1000), metadata: { couponCode: 'WELCOME20' } },
    { userId: customers[1]._id, title: 'Your order is being prepared 👨‍🍳', body: 'Burger Palace is preparing your order. Estimated delivery: 20 min.', channel: 'push', type: 'order_update', actionUrl: `/orders/${orders[6]._id}`, isRead: true, readAt: new Date(), sentAt: new Date(), metadata: { orderId: orders[6]._id.toString() } },
    ...
    { userId: customers[2]._id, title: 'You earned 100 loyalty points! ⭐', body: 'Welcome to our loyalty program! You\'ve been awarded 100 bonus points.', channel: 'in_app', type: 'loyalty', actionUrl: '/profile', isRead: true, readAt: new Date(now - 28 * 24 * 60 * 60 * 1000), sentAt: new Date(now - 30 * 24 * 60 * 60 * 1000), metadata: { points: 100 } },
