...
/**
 * /menu — Canonical SEO-clean menu page for Lassi Lounge.
 *
 * This is the primary public URL: lassiloungeny.com/menu
 * The old URL /restaurant/lassi-lounge redirects here via Next.js redirect().
 *
 * We load the restaurant using NEXT_PUBLIC_BRANDED_RESTAURANT_ID so no
 * slug is needed in the URL path.
 */

import { useEffect, useState, useRef } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import Fuse from 'fuse.js';
import { restaurantAPI, aiAPI, authAPI } from '@/lib/api';
...
export default function MenuPage() {
  ...
  const isOrderOnline = mode === 'delivery';
  ...
  const [couponApplied, setCouponApplied] = useState(false);   // ← yeh line add karo

  // Instantly trigger loading state BEFORE React commits to the DOM to prevent flickering
  if (mode !== prevMode) {
    setPrevMode(mode);
  ...
  useEffect(() => {
    if (isSwitchingMode) {
      ...
      const t = setTimeout(() => setIsSwitchingMode(false), 300);
  ...
  // Was missing entirely — DishGrid's heart/favorite button called this prop
  // directly with no fallback, so clicking it threw "toggleFavorite is not
  // a function" for any logged-in user.
  ...
  if (loading || isSwitchingMode) return <Loading />;
  ...
  return (
    <div className="min-h-screen bg-[#fdfbf7] flex flex-col font-sans">
      {isOrderOnline ? <OrderOnlineHero /> : <MenuHero />}
