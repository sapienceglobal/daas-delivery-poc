 ...
 * info, customization, add-to-cart panel, you-may-also-like, reviews,
 * value props). This is the SAME content that used to live directly inside
 * /restaurant/[id]/item/[itemId]/page.js.
 *
 * It now takes restaurantId/itemId as props instead of reading them from
 * useParams(), so BOTH routes — the enterprise-style
 * /restaurant/[id]/item/[itemId] route AND the single-restaurant
 * /item/[itemId] route — render this exact same component. No more drift
 * between two separate implementations.
 */
export default function ItemDetailContent({ restaurantId, itemId }) {
  ...
  return (
    <div className={`min-h-screen py-6 ${containerBg}`}>
      <div className="mx-auto max-w-[1400px] w-full px-4 md:px-6 lg:px-8 space-y-6">
        ...
        {/* MIDDLE SECTION: Customization Block */}
        <div className="pt-6">
          <CustomizationForm
            item={item}
