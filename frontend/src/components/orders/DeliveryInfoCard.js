...
export default function DeliveryInfoCard({ order }) {
  ...
  // Format estimated delivery time range
  const getEtaRange = () => {
    if (order.deliveryTime) {
      const etaDate = new Date(order.deliveryTime);
      const startEta = new Date(etaDate.getTime() - 10 * 60 * 1000); // 10 minutes earlier
    ...
    // Fallback: estimate 30-45 minutes from order creation
  ...
  // Live Map tracking progress calculations
  ...
  return (
    <div className="rounded-2xl border border-[#e5e7eb] bg-[#ffffff] p-6 shadow-sm space-y-6">
      ...
      {/* Embedded Live Map Tracking Block (Functionality fully preserved) */}
