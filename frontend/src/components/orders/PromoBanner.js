...
export default function PromoBanner({ isSingleRestaurantMode }) {
  const router = useRouter();

  if (!isSingleRestaurantMode) return null;
  ...
  return (
    ...
    >
      <h4 className="font-bold text-[18px] text-[#e8a020] mb-1.5">
      ...
      <button
        ...
        className="bg-[#e8a020] hover:bg-[#d68f13] text-[#1a1a1a] font-bold text-[12px] uppercase tracking-wider rounded-lg py-2.5 px-5 w-fit transition-colors"
      >
        JOIN NOW
      </button>
    </div>
