...
export default function LassiLoyaltyPage() {
  ...
  const handleJoinProgram = async () => {
    ...
    try {
      ...
      // FIX: this project's API client throws `err.message` directly
      // (see lib/api.js's `request()`) — it's a plain fetch wrapper, not
      // axios, so there's no `err.response.data`. The old
      // `err.response?.data?.message` check always evaluated to
      // undefined, silently swallowing whatever specific reason the
      // backend gave and always showing the generic fallback instead.
    ...
    return (
      <div key={coupon._id} className={`relative flex flex-col rounded-2xl border ${bgClass} shadow-sm hover:shadow-md transition-all overflow-hidden`}>
        ...
        <div className="p-6 flex flex-col h-full">
           ...
           {/* Expiry now shown right on the card face, not just buried
               inside the T&C modal — it's the thing people check first. */}
  ...
  return (
    <div className="min-h-screen bg-[#fbfaf7] pb-24">
      {/* Hero Section Banner — unchanged, kept consistent with every other page */}
      ...
      <main className="mx-auto max-w-6xl px-4 md:px-6 -mt-12 relative z-20 space-y-16">
        ...
        {/* Loyalty Status / Join Card (Floating) + progress bar */}
        <div className="space-y-4">
            ...
            {isMember ? (
              ...
              <div className="flex flex-col md:flex-row items-center justify-between w-full gap-6 text-center md:text-left">
                <div>
                  ...
                  {/* FIX: this used to hardcode "100 points = $1.00
                      discount", which contradicted the actual REWARDS
                      tiers below (100 points = $10 off). Now computed
                      from the same array so it can't drift out of sync. */}
          ...
          {/* Progress toward next reward — new. Gives members something
              concrete to work toward instead of just a raw points count,
              matching how Starbucks/Dunkin'-style loyalty programs do it. */}
          {isMember && nextReward && (
      ...
      {/* Redeem Confirmation Modal — replaces the old window.confirm(),
          which is a native browser prompt: can't be styled, and reads
          like a debug alert rather than part of the product. */}
      {confirmReward && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
