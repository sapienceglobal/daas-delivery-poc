...
export default function LiveOrdersView({ 
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="flex flex-1 gap-6 overflow-hidden pl-2">
          ...
          <div className="flex gap-4 h-full min-w-max">
            ...
            {columns.map(col => (
              <div key={col.id} className={`w-[270px] flex flex-col h-full rounded-2xl ${col.theme.bg} border ${col.theme.border} p-3`}>
                ...
                >
                  {col.orders.map(order => renderCard(order, col))}
                  {col.orders.length === 0 && (
        ...
        {/* Right Sidebar */}
        <div className="w-[300px] shrink-0 overflow-y-auto custom-scrollbar pr-4 space-y-6 pb-6">
