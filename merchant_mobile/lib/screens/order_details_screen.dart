...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  void _executeRemake(OrderModel order, bool chargeCustomer) async {
    ...
    try {
      final res = await ApiService.post('/api/orders/${order.id}/remake', {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Remake order sent to kitchen (${chargeCustomer ? 'Paid' : '\$0 charge'})',
            ),
          ),
        );
      ...
      if (chargeCustomer && resData['data'] != null) {
        ...
        final linkRes = await ApiService.post(
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to create remake order: $e')),
        );
    } finally {
  ...
  void _handleSendInvoice(OrderModel order) async {
    if (order.customerEmail == null || order.customerEmail!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No customer email on file')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Invoice emailed to ${order.customerEmail}')),
        );
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to send invoice email: $e')),
        );
    } finally {
  ...
  void _confirmCancel(OrderModel order) {
    _showPremiumDialog(
      title: 'Cancel Order',
      subtitle:
          'Are you sure you want to cancel this order? If paid, an auto-refund will be initiated.',
  ...
  void _confirmRefund(OrderModel order) {
    if (order.refunded) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Order has already been refunded.')),
      );
      return;
    }
    if (![
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Only paid orders can be refunded.')),
      );
      return;
    }
    _showPremiumDialog(
      title: 'Refund Order',
      subtitle:
          'Refund \$${order.total.toStringAsFixed(2)} to the customer? This action cannot be undone.',
      icon: Icons.history,
      ...
      onConfirm: () async {
        ...
        try {
          ...
          if (mounted)
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('Order Refunded')));
        } catch (e) {
          if (mounted)
            ScaffoldMessenger.of(
              context,
  ...
  Widget build(BuildContext context) {
    ...
    for (var su in order.statusUpdates) {
      ...
      if (su.description != null &&
          su.description!.toLowerCase().contains('refund of') &&
          su.description!.toLowerCase().contains('processed')) {
        label = 'refunded';
        key = 'refunded';
      }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _launchURL('/api/orders/${order.id}/kot'),
          ),
          IconButton(
            icon: const Icon(Icons.receipt, color: Colors.black),
            onPressed: () => _launchURL('/api/orders/${order.id}/invoice-pdf'),
          ),
        ],
      ),
                ...
                children: [
                  ...
                  if (order.hasAutoRefund) ...[
                    Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        color: order.autoRefundFailed
                            ? Colors.red.shade50
                            : order.autoRefundSkipped
                            ? Colors.blueGrey.shade50
                            : (order.autoRefundSucceeded
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  order.autoRefundFailed
                                      ? 'Auto-Refund Failed — Manual action required'
                                      : order.autoRefundSkipped
                                      ? 'Auto-Refund Disabled in Settings'
                                      : (order.autoRefundSucceeded
                              ...
                              Expanded(
                                child: Opacity(
                                  ...
                                  child: OutlinedButton.icon(
                                    onPressed: _isRemakingOrder
                                        ? null
                                        : () => _confirmRemake(order),
                                    icon: _isRemakingOrder
                                        ? const SizedBox(
                                            width: 16,
                                            height: 16,
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  'Order Failed — Payment may need manual review',
                                  style: GoogleFonts.inter(
                                    fontWeight: FontWeight.bold,
                                ...
                                Text(
                                  'Check the Order Journey below for details.',
                                  style: GoogleFonts.inter(
                                    fontSize: 12,
                                    color: Colors.red.shade700,
                                  ),
                              ...
                              children: [
                                Text(
                                  order.autoRefundFailed
                                      ...
                                      ? 'Auto-Refund Disabled in Settings'
                                      : order.autoRefundSucceeded
                                      ? '⚡ Auto-Refund Processed — \$${order.refundAmount > 0 ? order.refundAmount.toStringAsFixed(2) : order.total.toStringAsFixed(2)} was returned'
                                ...
                                if (order.refundReason != null &&
                                  ...
                                  Padding(
                                    padding: const EdgeInsets.only(top: 4),
                                    child: Text(
                                      'Reason: ${order.refundReason!.replaceAll('_', ' ')}',
                                      style: GoogleFonts.inter(
                                        fontSize: 12,
                                        fontWeight: FontWeight.w600,
                                        color: order.autoRefundFailed
                                ...
                                if (order.autoRefundSkipped)
                                  Padding(
                                    padding: const EdgeInsets.only(top: 4),
                                    child: Text(
                                      'You have turned off automatic refunds. Please initiate a manual refund below if required.',
                                      style: GoogleFonts.inter(
                                        fontSize: 12,
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildDeliveryRow(
                                  'Pickup Time',
                                  _formatDT(order.pickupTime),
                                ),
                                _buildDeliveryRow(
                                  'Delivery Time',
                                  _formatDT(order.deliveryTime),
                                ),
                                const SizedBox(height: 12),
                                const Divider(height: 1, color: Colors.black12),
                                const SizedBox(height: 12),
                                _buildDeliveryRow(
                                  'Delivery Provider',
                                  order.deliveryProvider == 'shipday'
                                      ? 'In-House'
                                      : (order.deliveryProvider != null
                                            ? order.deliveryProvider![0]
                                                      .toUpperCase() +
                                                  order.deliveryProvider!
                                                      .substring(1)
                                            : 'N/A'),
                                ),
                                if (order.thirdPartyDeliveryName != null)
                                  _buildDeliveryRow(
                                    '3rd Party Partner',
                                  ...
                                  if (order.courierPhoneForCustomer != null)
                                    Padding(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Text(
                                            'Rider Phone (Customer)',
                                            style: GoogleFonts.inter(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600),
                                          ),
                                          Row(
                                            children: [
                                              Text(order.courierPhoneForCustomer!, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold)),
                                              const SizedBox(width: 8),
                                              InkWell(
                                                onTap: () => launchUrl(Uri.parse('tel:${order.courierPhoneForCustomer}')),
                                                child: Container(
                                  ...
                                  if (order.courierPhoneForRestaurant != null)
                                    Padding(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Text(
                                            'Rider Phone (Restaurant)',
                                            style: GoogleFonts.inter(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600),
                                          ),
                                          Row(
                                            children: [
                                              Text(order.courierPhoneForRestaurant!, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold)),
                                              const SizedBox(width: 8),
                                              InkWell(
                                                onTap: () => launchUrl(Uri.parse('tel:${order.courierPhoneForRestaurant}')),
                                                child: Container(
                                                  padding: const EdgeInsets.all(4),
                                                  decoration: BoxDecoration(color: Colors.grey.shade100, shape: BoxShape.circle, border: Border.all(color: Colors.grey.shade300)),
                                                  child: const Icon(Icons.phone, size: 14, color: Colors.black87),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          if (order.tax > 0)
                            _buildFinancialRow('Tax', order.tax),
                          ...
                          if (order.tip > 0)
                            _buildFinancialRow('Tip', order.tip),
                          if (order.discount > 0)
                            _buildFinancialRow(
                              'Discount',
                              -order.discount,
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: journeyEvents.asMap().entries.map((entry) {
                          ...
                          final isRefundAction = ev['key'].toString().contains(
                            'refund',
                          );
                          final isFailedAction = ev['key'].toString().contains(
                            'fail',
                          );
                              ...
                              Column(
                                children: [
                                  ...
                                  if (!isLast)
                                    Container(
                                      width: 2,
                                      height: 32, // Line connecting to next
                                      margin: const EdgeInsets.only(right: 4),
                                      color: Colors.grey.shade200,
                                    ),
                                ],
                                    ...
                                    children: [
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            TimeUtils.formatDateTimeWithTz(
                                              ev['time'] as DateTime,
                                              TimeUtils.getRestaurantTimezone(Provider.of<AuthProvider>(context, listen: false).user),
                                            ).split('  ').last,
                                            style: GoogleFonts.inter(
                                              fontSize: 10,
                                              fontWeight: FontWeight.bold,
                                      ...
                                      if (ev['amount'] != null)
                                        Container(
                                          ...
                                          child: Text(
                                            '\$${(ev['amount'] as double).toStringAsFixed(2)}',
                                            style: GoogleFonts.inter(
                                              fontSize: 11,
                                              fontWeight: FontWeight.bold,
                                            ),
